...
export function extractReadFileEntry(event: ChatEvent): ReadFileTimelineEntry | null {
  ...
  if (summary) {
    ...
    if (/^failed to read\b/i.test(normalizedSummary)) {
      const target = normalizedSummary.replace(/^failed to read\s+/i, "").trim();
      return {
        label: `Failed to read ${shortenReadTargetForDisplay(target)}`,
        openPath: target.length > 0 ? target : null,
      };
    }
...
export function extractExploreRunKind(event: ChatEvent): ExploreRunKind | null {
  ...
  // Explore-like bash commands (find/tree/grep/etc.) are treated as search.
