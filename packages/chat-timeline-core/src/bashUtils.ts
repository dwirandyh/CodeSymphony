...
export function extractBashRuns(context: ChatEvent[]): BashRun[] {
  ...
  for (const event of ordered) {
    ...
    const hasFinalToolResult = run.status !== "running" && (run.output != null || run.error != null || (run.summary?.startsWith("Ran ") ?? false));
    const isLateSystemExpiryDeny = decision === "deny" && (message ?? "").includes("Session expired");

    if (decision === "deny") {
      if (hasFinalToolResult && isLateSystemExpiryDeny) {
        continue;
      }
      run.status = "failed";
