...
async function canonicalizeRepositoryPaths(
  repository: {
    id?: string;
    rootPath: string;
    worktrees: Array<{ id?: string; path: string; status?: "active" | "archived" | "deleting" | "delete_failed" }>;
  },
): Promise<void> {
...
export function createRepositoryService(prisma: PrismaClient) {
  type RepositoryWithWorktrees = {
    ...
    worktrees: Array<{
      ...
      path: string;
      baseBranch: string;
      status: "active" | "archived" | "deleting" | "delete_failed";
  ...
  async function syncWorktreeBranches(
    worktrees: Array<{ id: string; branch: string; path: string; status: "active" | "archived" | "deleting" | "delete_failed" }>,
  ): Promise<void> {
