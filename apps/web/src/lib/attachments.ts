...
const attachmentIdSession = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;

/**
 * Generate a session-local attachment ID.
 * These IDs only need to be unique within optimistic UI/editor flows.
 */
export function generateAttachmentId(): string {
  attachmentIdCounter += 1;
  return `attachment-${attachmentIdSession}-${attachmentIdCounter.toString(36)}`;
}
